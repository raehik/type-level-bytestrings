...

{
  ...
  let
    ...
    nondevDevShell = compiler: {
      mkShellArgs.name = "${compiler}-type-level-bytestrings";
      hoogle = false;
      tools = _: {
        hlint = null;
  ...
  in
    inputs.flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { self', pkgs, config, ... }: {
        packages.default  = self'.packages.ghc96-type-level-bytestrings;
        ...
        haskellProjects.ghc96 = {
          ...
          devShell.mkShellArgs.name = "ghc96-type-level-bytestrings";
        };
